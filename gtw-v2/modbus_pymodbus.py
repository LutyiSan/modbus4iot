...
class TCPClient:
    ...
    def connection(self, ip_address, tcp_port=502):
        try:
    ...
    def read_single(self, device):
        ...
        while idx < query_count:
            ...
            if device['reg_type'][idx] == "hr":
                pv = read_verifier(self.__read_hr(device['reg_address'][idx], device['quantity'][idx]),
                                   device['quantity'][idx])
                pv_list.append(pv)
            elif device['reg_type'][idx] == "ir":
                pv = read_verifier(self.__read_ir(device['reg_address'][idx], device['quantity'][idx]),
                                   device['quantity'][idx])
                pv_list.append(pv)
            elif device['reg_type'][idx] == "co":
                pv = read_verifier(self.__read_coils(device['reg_address'][idx], device['quantity'][idx]),
                                   device['quantity'][idx])
                pv_list.append(pv)
            elif device['reg_type'][idx] == "di":
                pv = read_verifier(self.__read_di(device['reg_address'][idx], device['quantity'][idx]),
                                   device['quantity'][idx])
                pv_list.append(pv)
    ...
    def read_multiple(self, signals):
        ...
        while idx < count - 1:
            ...
            if signals['reg_type'][idx] == "hr":
                pv = read_verifier(self.__read_hr(signals['start_address'][idx], signals['read_quantity'][idx]),
                                   signals['read_quantity'][idx])
                pv_list.append(pv)
            elif signals['reg_type'][idx] == "ir":
                pv_list.append(
                    read_verifier(self.__read_ir(signals['start_address'][idx], signals['read_quantity'][idx]),
                                  signals['read_quantity'][idx]))
            elif signals['reg_type'][idx] == "coil":
                pv_list.append(
                    read_verifier(self.__read_coils(signals['start_address'][idx], signals['read_quantity'][idx]),
                                  signals['read_quantity'][idx]))
            elif signals['reg_type'][idx] == "di":
                pv_list.append(
                    read_verifier(self.__read_di(signals['start_address'][idx], signals['read_quantity'][idx]),
                                  signals['read_quantity'][idx]))
        logger.info(".....STOP reading")
        for group in pv_list:
            for value in group:
