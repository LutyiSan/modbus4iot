...
def to_float(values):
    b = bytearray(4)
    b[0] = values[1] & 0xff
    b[1] = (values[1] & 0xff00) >> 8
    b[2] = (values[0] & 0xff)
    b[3] = (values[0] & 0xff00) >> 8
    ...
    def convert(self):
        ...
        while i < count:
            ...
            if self.bit_number[i] == 'None' or (self.bit_number[i] != 'None' and self.value_type[i] != 'bool'):
                ...
                #   print("elif-1  ", i)
                add_quantity = 1
                if self.reg_address[i] != self.reg_address[i + 1]:
                    pv = to_bool(self.data_values[index_data_value:index_data_value + 1], self.bit_number[i])
                    self.present_value.append(pv)
                    i += 1
        ...
        # print(f"len signals {self.signals['name']}  len data{self.signals['present_value']}")
        return self.signals
